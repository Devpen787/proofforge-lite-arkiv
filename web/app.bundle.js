...
var init_transaction = __esm({
  "node_modules/viem/_esm/errors/transaction.js"() {
    ...
    TransactionReceiptRevertedError = class extends BaseError2 {
      constructor({ receipt: receipt2 }) {
        super(`Transaction with hash "${receipt2.transactionHash}" reverted.`, {
          metaMessages: [
...
async function localBatchGatewayRequest(parameters) {
  ...
  const failures = [];
  const responses = [];
  await Promise.all(queries.map(async (query2, i) => {
    try {
      responses[i] = query2.urls.includes(localBatchGatewayUrl) ? await localBatchGatewayRequest({ data: query2.data, ccipRequest: ccipRequest2 }) : await ccipRequest2(query2);
      failures[i] = false;
    } catch (err) {
      failures[i] = true;
...
var wasm, heap, cachedTextDecoder, cachegetUint8Memory0, heap_next, WASM_VECTOR_LEN, cachedTextEncoder, encodeString3, cachegetInt32Memory0, stack_pointer, BrotliStreamResultCode, BrotliStreamResult, CompressStream, DecompressStream, brotli_wasm_default;
...
// data/public-safe-packet.json
var public_safe_packet_default = {
  packetId: "public_packet_arkiv_ethns_submission",
  workItemId: "work_arkiv_ethns_submission",
  mission: "Prepare ProofForge Lite Arkiv submission",
  whatWasProven: "A bounded proof-node workflow converted a private hackathon build process into a public-safe proof packet for the Arkiv ETHNS challenge.",
  evidenceSummary: "Requirements mapped, public/private boundary checked, Arkiv schema verified, demo path prepared, and release candidate built locally. Live Braga writes remain approval-gated.",
  artifactHashes: [
    "sha256:9f2a6b1a0b8c1f4a3a7d6e1b0c2d4e5f6a7b8c9d0e1f2031425364758697a8b9",
    "sha256:6c3e5f9268a2e3f4a31e6f7c7c9b1e3b90820d84d84dbed5aa2d5c03631a4a8d"
  ],
  sourceUrl: "https://ns.com/earn/web3-database-builder-challenge",
  sourceType: "hackathon_challenge",
  status: "ready_for_submission",
  reviewDecision: "ready_for_submission",
  riskScore: 18,
  evidenceCount: 7,
  createdAtMs: 1779627e6,
  acceptedAtMs: 0,
  publicFields: [
    "challenge source",
...
var seed_work_items_default = [
  {
    workItemId: "work_arkiv_ethns_submission",
    title: "Arkiv ETHNS Web3 Database Builder Challenge",
    source: "Network School / Arkiv",
    sourceUrl: "https://ns.com/earn/web3-database-builder-challenge",
    sourceType: "hackathon_challenge",
    status: "ready_for_submission",
    priority: 1,
    acceptanceOwner: "Arkiv / Network School reviewers",
    valuePath: "USDC prize claim if accepted externally",
    proofNeeded: "Public repo, working Braga demo, README setup, live Arkiv entities, project-scoped queries, and 2-3 minute demo video.",
    publicBoundary: "Only challenge requirements, public-safe proof summaries, artifact hashes, and Arkiv refs are public.",
    riskScore: 18,
    evidenceCount: 7,
    createdAtMs: 1779627e6,
    deadlineMs: 177975354e4
  },
  {
    workItemId: "work_protocol_bounty_sample",
    title: "Protocol bounty proof pack",
    source: "Public bounty board sample",
    sourceUrl: "sample://protocol-bounty",
    sourceType: "bounty",
    status: "queued",
    priority: 2,
    acceptanceOwner: "Protocol maintainer",
    valuePath: "External/manual payout if accepted",
    proofNeeded: "Source requirements, bounded proof run, verifier result, redacted maintainer packet.",
    publicBoundary: "Sample only. No private bounty intelligence or payout details.",
    riskScore: 32,
    evidenceCount: 3,
    createdAtMs: 177962706e4,
    deadlineMs: 177984e7
  },
  {
    workItemId: "work_grant_review_sample",
    title: "Grant application proof review",
    source: "Foundation grant sample",
    sourceUrl: "sample://foundation-grant",
    sourceType: "grant",
    status: "queued",
    priority: 3,
    acceptanceOwner: "Foundation reviewer",
    valuePath: "Credit, access, or grant decision if accepted",
    proofNeeded: "Eligibility checks, public-safe claim review, evidence hash list, reviewer decision event.",
    publicBoundary: "Sample only. Draft applications and private notes stay outside Arkiv.",
    riskScore: 41,
    evidenceCount: 2,
    createdAtMs: 177962712e4,
    deadlineMs: 17799264e5
  }
];
...
function formatTransactionReceipt(transactionReceipt, _) {
  ...
  if (transactionReceipt.blobGasPrice)
    receipt2.blobGasPrice = BigInt(transactionReceipt.blobGasPrice);
...
async function getCallsStatus(client, parameters) {
  ...
  return {
    ...
    receipts: receipts?.map((receipt2) => ({
      ...receipt2,
      blockNumber: hexToBigInt(receipt2.blockNumber),
      gasUsed: hexToBigInt(receipt2.gasUsed),
      status: receiptStatuses[receipt2.status]
    })) ?? [],
    statusCode,
    status,
    version: version4
...
async function getTransactionReceipt(client, { hash: hash3 }) {
  ...
  return format(receipt2, "getTransactionReceipt");
}
  ...
  _unobserve = observe(observerId, { onReplaced, resolve, reject }, async (emit) => {
    ...
    _unwatch = getAction(client, watchBlockNumber, "watchBlockNumber")({
      ...
      async onBlockNumber(blockNumber_) {
        ...
        try {
          if (receipt2) {
            if (confirmations > 1 && (!receipt2.blockNumber || blockNumber - receipt2.blockNumber + 1n < confirmations))
              return;
            done(() => emit.resolve(receipt2));
            return;
          }
          ...
          receipt2 = await getAction(client, getTransactionReceipt, "getTransactionReceipt")({ hash: hash3 });
          if (confirmations > 1 && (!receipt2.blockNumber || blockNumber - receipt2.blockNumber + 1n < confirmations))
            return;
          done(() => emit.resolve(receipt2));
        } catch (err) {
          if (err instanceof TransactionNotFoundError || err instanceof TransactionReceiptNotFoundError) {
            ...
            try {
              ...
              if (!replacementTransaction)
                return;
              receipt2 = await getAction(client, getTransactionReceipt, "getTransactionReceipt")({
...
async function sendRawTransactionSync(client, { serializedTransaction, throwOnReceiptRevert, timeout }) {
  ...
  const format = client.chain?.formatters?.transactionReceipt?.format || formatTransactionReceipt;
  const formatted = format(receipt2);
...
async function sendTransactionSync(client, parameters) {
  ...
  try {
    ...
    if (account?.type === "json-rpc" || account === null) {
      ...
      const receipt2 = await getAction(client, waitForTransactionReceipt, "waitForTransactionReceipt")({
        checkReplacement: false,
        hash: hash3,
        pollingInterval,
        timeout
      });
      if (throwOnReceiptRevert && receipt2.status === "reverted")
        throw new TransactionReceiptRevertedError({ receipt: receipt2 });
      return receipt2;
    }
    if (account?.type === "local") {
      if (account.nonceManager && typeof nonce === "undefined") {
...
// node_modules/@arkiv-network/sdk/dist/chains-CrtkVbBh.js
...
var chains_exports = /* @__PURE__ */ __export2({
  braga: () => braga,
  kaolin: () => kaolin,
  localhost: () => localhost
});

// node_modules/@arkiv-network/sdk/dist/queryBuilder-DonAg1aq.js
var ROOT_NAMESPACE = "arkiv";
function getDebugEnv() {
  if (typeof process !== "undefined") {
    const debugEnv = process?.env?.DEBUG;
    if (debugEnv) return debugEnv;
  }
...
function matchesPattern(namespace, pattern) {
  ...
  if (pattern.startsWith("-")) return !matchesPattern(namespace, pattern.slice(1));
  return false;
}
function isEnabled(namespace) {
  ...
  let enabled = false;
  for (const pattern of patterns) if (pattern.startsWith("-")) {
    if (matchesPattern(namespace, pattern)) return false;
  } else if (matchesPattern(namespace, pattern)) enabled = true;
  return enabled;
}
var enabledNamespaces = /* @__PURE__ */ new Set();
var disabledNamespaces = /* @__PURE__ */ new Set();
var COLOR_PALETTE = [
  "#0000FF",
  "#008000",
  "#FF0000",
...
var namespaceColors = /* @__PURE__ */ new Map();
function getColorForNamespace(namespace) {
  ...
  const color = {
    css: COLOR_PALETTE[colorIndex],
    ansi: ANSI_COLORS[colorIndex % ANSI_COLORS.length]
  };
...
function createDebugLogger(namespace) {
  ...
  return (...args) => {
    ...
    if (envEnabled || progEnabled) if (isNode) {
      ...
      console.log(`%c${namespace}%c`, style1, "color: inherit; font-weight: normal", ...args);
    }
  };
}
var getLogger = (scope) => {
  return createDebugLogger(`${ROOT_NAMESPACE}:${scope}`);
};
var EntityMutationError = class extends Error {
  constructor(message) {
    super(message);
...
var Entity = class {
  ...
  payload;
  attributes;
  constructor(key, contentType = void 0, owner = void 0, creator = void 0, expiresAtBlock = void 0, createdAtBlock = void 0, lastModifiedAtBlock = void 0, transactionIndexInBlock = void 0, operationIndexInTransaction = void 0, payload = void 0, attributes) {
    this.key = key;
    this.owner = owner;
  ...
  */
  toText() {
    if (this.payload === void 0) throw new Error("Entity has no payload \u2013 it was probably queried without withPayload(true) via QueryBuilder");
    try {
  ...
  */
  toJson() {
    ...
    if (!text) throw new Error("Entity has empty payload, cannot parse as JSON");
    try {
...
var logger$2 = getLogger("utils:entities");
async function entityFromRpcResult(rpcEntity) {
  logger$2("entityFromRpcResult %o", rpcEntity);
  return new Entity(rpcEntity.key, rpcEntity.contentType, rpcEntity.owner, rpcEntity.creator, rpcEntity.expiresAt !== void 0 ? BigInt(rpcEntity.expiresAt) : void 0, rpcEntity.createdAtBlock !== void 0 ? BigInt(rpcEntity.createdAtBlock) : void 0, rpcEntity.lastModifiedAtBlock !== void 0 ? BigInt(rpcEntity.lastModifiedAtBlock) : void 0, rpcEntity.transactionIndexInBlock !== void 0 ? BigInt(rpcEntity.transactionIndexInBlock) : void 0, rpcEntity.operationIndexInTransaction !== void 0 ? BigInt(rpcEntity.operationIndexInTransaction) : void 0, rpcEntity.value !== void 0 ? toBytes(rpcEntity.value) : void 0, [...(rpcEntity.stringAttributes ?? []).map(({ key, value }) => ({
    key,
    value
...
var logger$1 = getLogger("query:engine");
function processPredicates(predicates) {
  ...
  return predicates.map((predicate) => {
    switch (predicate.type) {
      ...
      case "or":
        return `(${predicate.predicates.map((predicate$1) => processPredicates([predicate$1])).join(" || ")})`;
      case "and":
        return `(${predicate.predicates.map((predicate$1) => processPredicates([predicate$1])).join(" && ")})`;
      default:
        return "";
...
async function processQuery(client, queryParams) {
  const { predicates, limit, cursor, ownedBy, createdBy, orderBy, validAtBlock, withAttributes, withMetadata, withPayload } = queryParams;
  logger$1("Processing query with params %o", {
    predicates,
    cursor,
    limit,
    ownedBy,
  ...
  if (ownedBy) query2 += ` && $owner=${ownedBy}`;
  if (createdBy) query2 += ` && $creator=${createdBy}`;
  query2 = query2.trim();
  if (query2.startsWith("&& ")) query2 = query2.slice(3);
  ...
  if (validAtBlock !== void 0) queryOptions.atBlock = numberToHex(validAtBlock);
  if (limit !== void 0) queryOptions.resultsPerPage = numberToHex(limit);
  if (cursor !== void 0) queryOptions.cursor = cursor;
  if (orderBy !== void 0) queryOptions.orderBy = orderBy;
  logger$1("Built query to send %s %o", query2, {
    includeData: queryOptions.includeData,
    atBlock: queryOptions.atBlock ? hexToNumber(queryOptions.atBlock) : void 0,
  ...
  logger$1("Raw result from query %o", result);
  return result;
}
var logger = getLogger("query:result");
...
var QueryBuilder = class {
  ...
  */
  ...
  orderBy(attributeNameOrOrderByAttribute, attributeType, order = "asc") {
    ...
    const pushOrderByAttribute = ({ name, type, order: order$1 }) => {
      this._orderBy?.push({
        name,
    ...
    if (typeof attributeNameOrOrderByAttribute === "string") {
      if (!attributeType) throw new Error("attributeType is required when using positional orderBy arguments");
      pushOrderByAttribute({
        name: attributeNameOrOrderByAttribute,
        type: attributeType,
        order
      });
    } else pushOrderByAttribute(attributeNameOrOrderByAttribute);
    return this;
  }
  /**
...
// node_modules/@arkiv-network/sdk/dist/payload-DKBYYjeA.js
var ARKIV_ADDRESS = "0x00000000000000000000000000000061726b6976";
var BLOCK_TIME = 2;
function jsonToPayload(json) {
  return toBytes(JSON.stringify(json));
}

// node_modules/@arkiv-network/sdk/dist/index.js
var logger$9 = getLogger("actions:public:get-block-timing");
async function getBlockTiming(client) {
  const blockTiming = await client.request({
    method: "arkiv_getBlockTiming",
    params: []
  });
  logger$9("Block timing %o", blockTiming);
  return {
...
async function getEntity(client, key) {
  ...
  if (bytes.length !== 32) throw new Error(`Invalid key length: ${bytes.length} bytes. Key must be 32 bytes (64 hex characters) long.`);
  const hexKey = bytesToHex(bytes);
  const result = await client.request({
    method: "arkiv_query",
    params: [`$key = ${hexKey}`, { includeData: {
      key: true,
      attributes: true,
      payload: true,
      contentType: true,
  ...
  if (!result.data.length) throw new NoEntityFoundError();
...
async function query(client, query$1, queryOptions) {
  const rpcQueryOptions = {
    ...
    ...queryOptions?.atBlock !== void 0 && { atBlock: numberToHex(queryOptions.atBlock) },
    ...queryOptions?.orderBy && { orderBy: queryOptions?.orderBy.map((order) => ({
      name: order.name,
      type: order.type,
      desc: order.desc === "desc"
    })) },
    ...queryOptions?.resultsPerPage !== void 0 && { resultsPerPage: numberToHex(queryOptions.resultsPerPage) },
    ...queryOptions?.cursor !== void 0 && { cursor: queryOptions?.cursor }
  };
  const result = await client.request({
    method: "arkiv_query",
    params: [query$1, rpcQueryOptions]
  });
  return {
    entities: await Promise.all(result.data.map((entity) => entityFromRpcResult(entity))),
    cursor: result.cursor,
    blockNumber: BigInt(result.blockNumber)
  };
}
var logger$8 = getLogger("actions:public:subscribe-entity-events");
var arkivABI = parseAbi([
...
async function subscribeEntityEvents(client, { onError, onEntityCreated, onEntityUpdated, onEntityDeleted, onEntityExpired, onEntityExpiresInExtended, onEntityOwnerChanged }, pollingInterval, fromBlock) {
  return client.watchEvent({
    pollingInterval: pollingInterval ?? 1e3,
    fromBlock,
    events: arkivABI,
    onLogs: (logs) => {
      logger$8("logs from subscribeEntityEvents %o", logs);
      for (const log of logs) {
        ...
        logger$8("event from subscribeEntityEvents %o", event);
...
function createPublicClient(parameters) {
  ...
  return createClient({
    ...
    key,
    name
  }).extend(publicActions).extend(publicArkivActions);
}
var logger$7 = getLogger("utils:compression");
var isNode2 = typeof process !== "undefined" && process.versions?.node != null;
var zlib = null;
var brotli = null;
async function getZlib() {
  if (!isNode2) return null;
  if (!zlib) try {
...
async function getBrotli() {
  ...
  try {
    const brotliModule = await Promise.resolve().then(() => (init_index_web(), index_web_exports));
...
async function compress2(data) {
  logger$7("Compressing data size %d", data.length);
  if (isNode2) {
    ...
    if (zlibModule) try {
      ...
      logger$7("Brotli compressed with zlib size %d", result.length);
      return result;
    } catch (error) {
  ...
  logger$7("Brotli compressed with wasm size %d", brotliCompressed.length);
  return brotliCompressed;
}
var logger$6 = getLogger("utils:arkiv-transactions");
function opsToTxData({ creates, updates, deletes, extensions, ownershipChanges }) {
  function formatAttributes(attribute) {
    return [toHex(attribute.key), toHex(typeof attribute.value === "number" && attribute.value === 0 ? "" : attribute.value)];
  }
  const payload = [
    (creates ?? []).map((item) => [
    ...
    (extensions ?? []).map((item) => [item.entityKey, toHex(Math.ceil(item.expiresIn / BLOCK_TIME))]),
    (ownershipChanges ?? []).map((item) => [item.entityKey, item.newOwner])
  ];
  logger$6("txData to send as RLP %o length %d", payload, payload.length);
...
async function sendArkivTransaction(client, data, txParams) {
  ...
  const walletClient2 = client;
  logger$6("Sending transaction %o", {
  ...
  try {
    ...
    const receipt2 = await walletClient2.waitForTransactionReceipt({ hash: txHash });
    logger$6("Tx receipt %o", receipt2);
    if (receipt2.status === "reverted") try {
      const callResult = await walletClient2.call({
        account: client.account,
        to: ARKIV_ADDRESS,
        value: 0n,
        data: toHex(compressed),
      ...
      throw new EntityMutationError(`Transaction ${receipt2.transactionHash} reverted. Please make sure the data for entities doesn't contain invalid characters or invalid data types.
          Reason: ${callResult.data ? hexToString(callResult.data) : "No reason provided by backend."}`);
    } catch (err) {
      const reason = err.cause?.details ?? "No reason provided by backend.";
      logger$6("Error while calling to get more details about reverted transaction. Reason: %s", reason);
      throw new EntityMutationError(`Transaction ${receipt2.transactionHash} reverted. Reason: ${reason}`);
    }
    return receipt2;
  } catch (error) {
    let message = "Transaction failed";
    if (error instanceof TransactionExecutionError) message += `: ${error.details}`;
    else if (error instanceof EntityMutationError) throw error;
    logger$6("%s Detailed error stack: %o", message, error);
    throw new EntityMutationError(message);
  }
}
var logger$5 = getLogger("actions:wallet:change-ownership");
async function changeOwnership(client, data, txParams) {
  logger$5("changeOwnership %o", data);
  const receipt2 = await sendArkivTransaction(client, opsToTxData({ ownershipChanges: [data] }), txParams);
  logger$5("Receipt from changeOwnership %o", receipt2);
  return {
    txHash: receipt2.transactionHash,
    entityKey: data.entityKey
  };
}
var logger$4 = getLogger("actions:wallet:create-entity");
async function createEntity(client, data, txParams) {
  logger$4("createEntity %o", data);
  const receipt2 = await sendArkivTransaction(client, opsToTxData({ creates: [data] }), txParams);
  logger$4("Receipt from createEntity %o", receipt2);
  return {
    txHash: receipt2.transactionHash,
    entityKey: receipt2.logs[0].topics[1]
  };
}
var logger$3 = getLogger("actions:wallet:delete-entity");
async function deleteEntity(client, data, txParams) {
  logger$3("deleteEntity %o", data);
  const receipt2 = await sendArkivTransaction(client, opsToTxData({ deletes: [data] }), txParams);
  logger$3("Receipt from deleteEntity %o", receipt2);
  return {
    txHash: receipt2.transactionHash,
    entityKey: data.entityKey
  };
}
var logger$22 = getLogger("actions:wallet:extend-entity");
async function extendEntity(client, data, txParams) {
  logger$22("extendEntity %o", data);
  const receipt2 = await sendArkivTransaction(client, opsToTxData({ extensions: [data] }), txParams);
  logger$22("Receipt from extendEntity %o", receipt2);
  return {
    txHash: receipt2.transactionHash,
    entityKey: receipt2.logs[0].topics[1]
  };
}
var logger$12 = getLogger("actions:wallet:mutate-entities");
function parseReceipt(receipt2, params) {
  ...
  for (let index2 = 0; index2 < receipt2.logs.length; index2++) {
    const log = receipt2.logs[index2];
    if (index2 < totalCreates) createdEntities.push(log.topics[1]);
    else if (index2 < totalCreates + totalDeletes) deletedEntities.push(log.topics[1]);
    else if (index2 < totalCreates + totalUpdates + totalDeletes) updatedEntities.push(log.topics[1]);
    else if (index2 < totalCreates + totalUpdates + totalDeletes + totalExtensions) extendedEntities.push(log.topics[1]);
    else ownershipChanges.push(log.topics[1]);
  }
  return {
    createdEntities,
    updatedEntities,
...
async function mutateEntities(client, data, txParams) {
  ...
  const receipt2 = await sendArkivTransaction(client, opsToTxData({
  ...
  logger$12("Receipt from mutateEntities %o", receipt2);
  const { createdEntities, updatedEntities, deletedEntities, extendedEntities, ownershipChanges } = parseReceipt(receipt2, data);
  return {
    txHash: receipt2.transactionHash,
    createdEntities,
    updatedEntities,
...
var logger2 = getLogger("actions:wallet:update-entity");
async function updateEntity(client, data, txParams) {
  logger2("updateEntity %o", data);
  const receipt2 = await sendArkivTransaction(client, opsToTxData({ updates: [data] }), txParams);
  logger2("Receipt from updateEntity %o", receipt2);
  return {
    txHash: receipt2.transactionHash,
    entityKey: receipt2.logs[0].topics[1]
  };
...
function createWalletClient(parameters) {
  ...
  return createClient({
    ...
    key,
    name
  }).extend(publicActions).extend(walletActions).extend(walletArkivActions);
}
var chainMap = new Map(Object.entries(chains_exports).map(([key, value]) => [key.toLowerCase(), value]));

// node_modules/@arkiv-network/sdk/dist/query/index.js
...
// node_modules/@arkiv-network/sdk/dist/utils/index.js
var ExpirationTime = {
  fromSeconds: (seconds) => seconds,
  fromMinutes: (minutes) => minutes * 60,
  fromHours: (hours) => hours * 60 * 60,
  fromDays: (days) => days * 24 * 60 * 60,
  fromWeeks: (weeks) => weeks * 7 * 24 * 60 * 60,
  fromMonths: (months) => months * 30 * 24 * 60 * 60,
  fromYears: (years) => years * 365 * 24 * 60 * 60,
  fromBlocks: (blocks) => blocks * BLOCK_TIME,
  fromDate: (date) => Math.floor(date.getTime() / 1e3) - Math.floor(Date.now() / 1e3)
};
...
var EXPIRATION_TIERS = {
  workQueueDays: 14,
  submittedPacketDays: 30,
  acceptedProofDays: 90,
  reviewEventDays: 45
};
function buildProofWorkspaceEntity({ nowMs, workItemCount = 3 }) {
  return {
...
function buildWorkItemEntity({ workItem, proofWorkspaceKey }) {
  return {
...
function buildProofPacketEntity({
  packet,
  proofWorkspaceKey,
  workItemKey
}) {
  return {
    ...
    expirationTier: packet.status === "accepted" ? "acceptedProofDays" : "submittedPacketDays"
  };
}
function buildReviewEventEntity({
  packet,
  proofWorkspaceKey,
  workItemKey,
  reviewedAtMs
  ...
  return {
    ...
    payload: {
      ...
      reviewSummary: "Ready for public challenge review. Prize acceptance and official submission remain external approval-gated actions."
    },
    contentType: "application/json",
    attributes: [
      PROJECT_ATTRIBUTE,
...
var REQUIRED_QUERIES = [
  ...
  },
  {
    ...
    filters: [
      PROJECT_ATTRIBUTE,
      { key: "type", value: ENTITY_TYPES.ProofPacketSummary },
      { key: "createdAtMs", op: "gte", value: 17795406e5 }
    ]
  },
  {
...
var BRAGA_CHAIN_ID_HEX = `0x${braga.id.toString(16)}`;
function createArkivPublicClient() {
  return createPublicClient({
    chain: braga,
    transport: http()
  });
}
async function ensureBragaWalletChain(ethereum = window.ethereum) {
  if (!ethereum?.request) {
...
function toArkivCreatePayload(entity) {
  return {
...
async function publishProofMemory(walletClient2, { packet, workItem, workItems = [workItem], nowMs = Date.now() }) {
  ...
  const proofWorkspace2 = buildProofWorkspaceEntity({
    nowMs,
    workItemCount: workItems.length
  });
  const proofWorkspaceWrite = await walletClient2.createEntity(
    toArkivCreatePayload(proofWorkspace2)
  );
  const workItemEntities2 = workItems.map(
    (candidate) => buildWorkItemEntity({
      workItem: candidate,
      proofWorkspaceKey: proofWorkspaceWrite.entityKey
    })
  );
  const workItemWrites = await walletClient2.mutateEntities({
    creates: workItemEntities2.map(toArkivCreatePayload)
  });
  ...
  const proofPacket2 = buildProofPacketEntity({
    packet,
    proofWorkspaceKey: proofWorkspaceWrite.entityKey,
    workItemKey: selectedWorkItemEntityKey
  });
  const reviewEvent2 = buildReviewEventEntity({
    packet,
    proofWorkspaceKey: proofWorkspaceWrite.entityKey,
    workItemKey: selectedWorkItemEntityKey,
    reviewedAtMs: nowMs
  });
  const childWrites = await walletClient2.mutateEntities({
    creates: [proofPacket2, reviewEvent2].map(toArkivCreatePayload)
  });
  return {
...
async function queryWorkItemsByStatus(publicClient, status) {
  return publicClient.buildQuery().where(eq(PROJECT_ATTRIBUTE.key, PROJECT_ATTRIBUTE.value)).where(eq("type", ENTITY_TYPES.WorkItem)).where(eq("status", status)).withPayload(true).withAttributes(true).withMetadata(true).limit(20).fetch();
}
async function queryWorkItemsByProject(publicClient) {
  return publicClient.buildQuery().where(eq(PROJECT_ATTRIBUTE.key, PROJECT_ATTRIBUTE.value)).where(eq("type", ENTITY_TYPES.WorkItem)).withPayload(true).withAttributes(true).withMetadata(true).limit(20).fetch();
}
async function queryProofPacketsByStatus(publicClient, status) {
  return publicClient.buildQuery().where(eq(PROJECT_ATTRIBUTE.key, PROJECT_ATTRIBUTE.value)).where(eq("type", ENTITY_TYPES.ProofPacketSummary)).where(eq("status", status)).withPayload(true).withAttributes(true).withMetadata(true).limit(20).fetch();
}
async function queryProofPacketsBySourceType(publicClient, sourceType) {
  return publicClient.buildQuery().where(eq(PROJECT_ATTRIBUTE.key, PROJECT_ATTRIBUTE.value)).where(eq("type", ENTITY_TYPES.ProofPacketSummary)).where(eq("sourceType", sourceType)).withPayload(true).withAttributes(true).withMetadata(true).limit(20).fetch();
}
async function queryRecentProofPackets(publicClient, minCreatedAtMs) {
  return publicClient.buildQuery().where(eq(PROJECT_ATTRIBUTE.key, PROJECT_ATTRIBUTE.value)).where(eq("type", ENTITY_TYPES.ProofPacketSummary)).where(gte("createdAtMs", minCreatedAtMs)).withPayload(true).withAttributes(true).withMetadata(true).limit(20).fetch();
}
async function queryReviewEventsByProofWorkspace(publicClient, proofWorkspaceKey) {
  return publicClient.buildQuery().where(eq(PROJECT_ATTRIBUTE.key, PROJECT_ATTRIBUTE.value)).where(eq("type", ENTITY_TYPES.ReviewEvent)).where(eq("proofWorkspaceKey", proofWorkspaceKey)).withPayload(true).withAttributes(true).withMetadata(true).limit(20).fetch();
}
async function queryReviewEventsByWorkItem(publicClient, workItemKey) {
  return publicClient.buildQuery().where(eq(PROJECT_ATTRIBUTE.key, PROJECT_ATTRIBUTE.value)).where(eq("type", ENTITY_TYPES.ReviewEvent)).where(eq("workItemKey", workItemKey)).withPayload(true).withAttributes(true).withMetadata(true).limit(20).fetch();
}

// src/browser-app.ts
var selectedWorkItem = seed_work_items_default.find((workItem) => workItem.workItemId === public_safe_packet_default.workItemId) ?? seed_work_items_default[0];
var proofWorkspace = buildProofWorkspaceEntity({
  nowMs: public_safe_packet_default.createdAtMs,
  workItemCount: seed_work_items_default.length
});
var workItemEntities = seed_work_items_default.map(
  (candidate) => buildWorkItemEntity({
    workItem: candidate,
    proofWorkspaceKey: "pending_wallet_write_approval"
  })
);
var proofPacket = buildProofPacketEntity({
  packet: public_safe_packet_default,
  proofWorkspaceKey: "pending_wallet_write_approval",
  workItemKey: "pending_wallet_write_approval"
});
var reviewEvent = buildReviewEventEntity({
  packet: public_safe_packet_default,
  proofWorkspaceKey: "pending_wallet_write_approval",
  workItemKey: "pending_wallet_write_approval",
  reviewedAtMs: public_safe_packet_default.createdAtMs + 6e4
});
var receipt = {
...
var publicPrivateSplit = {
  publishedToArkiv: public_safe_packet_default.publicFields,
  keptPrivate: public_safe_packet_default.privateFieldsExcluded
};
var schemaView = {
  projectAttribute: PROJECT_ATTRIBUTE,
...
var stepTabs = Array.from(document.querySelectorAll(".step-tab"));
...
var proofRunTraceList = requiredElement("#proofRunTrace");
...
function renderWorkRows() {
  workRows.innerHTML = seed_work_items_default.map((workItem) => {
    ...
    return `
        <article class="work-row ${selected ? "selected" : ""}">
          <div>
            ...
            <span>${workItem.source} \xB7 ${workItem.acceptanceOwner}</span>
          </div>
          <b>${workItem.status.replaceAll("_", " ")}</b>
          <span>${risk} \xB7 ${workItem.evidenceCount} evidence refs</span>
...
function renderProofRunTrace() {
  proofRunTraceList.innerHTML = proofRunTrace.map(
    (event, index2) => `
        <span>
          <small>${index2 + 1}</small>
          ${event}
        </span>
      `
  ).join("");
...
function renderEvidenceSummary(value, title) {
  ...
  if (title === "Live Arkiv write receipt" && value && typeof value === "object") {
    ...
    const relationshipCount = Number(queryEvidence.reviewEventsByWorkspaceCount ?? 0) + Number(queryEvidence.reviewEventsByWorkItemCount ?? 0);
    evidenceSummary.innerHTML = [
...
function extractResultCount(result) {
  if (result && typeof result === "object" && "entities" in result) {
    ...
    return Array.isArray(entities) ? entities.length : void 0;
  }
  return Array.isArray(result) ? result.length : void 0;
}
function firstEntity(result) {
  if (result && typeof result === "object" && "entities" in result) {
    const entities = result.entities;
    return Array.isArray(entities) ? entities[0] : void 0;
  }
  return Array.isArray(result) ? result[0] : void 0;
}
function extractEntityKey(entity) {
...
function extractMetadataAddressEvidence(result, field, queryName) {
...
async function buildLiveWriteResult(writeResult) {
  ...
  const ownerEvidence = extractMetadataAddressEvidence(
    reviewEventsByWorkItem,
    "owner",
    "reviewEventsByWorkItem"
  );
  const creatorEvidence = extractMetadataAddressEvidence(
    reviewEventsByWorkItem,
    "creator",
    "reviewEventsByWorkItem"
  );
  return {
    ...
    queryEvidence: {
      ...
      reviewEventsByWorkspaceCount: extractResultCount(reviewEventsByWorkspace) ?? 0,
  ...
  setReceiptView(
    {
      projectAttribute: PROJECT_ATTRIBUTE,
      arkivEntityPlan: {
        durablePublicData: "After approval, every durable public ProofForge Lite record is written as a project-scoped Arkiv entity.",
        entityTypes: [
          "proof_workspace",
          "work_item",
        ...
        selectedWorkItemRelationship: "proof_packet_summary.workItemKey -> work_item.$key"
      },
      selectedWorkItem,
      packet: public_safe_packet_default,
      publicPrivateSplit
...
requiredElement("#inspectSchema").addEventListener("click", () => {
...
liveWrite.addEventListener("click", async () => {
  try {
    ...
    const writeResult = await publishProofMemory(walletClient, {
      ...
      workItems: seed_work_items_default,
      nowMs: Date.now()
    });
...
requiredElement("#copyReceipt").addEventListener("click", async () => {
